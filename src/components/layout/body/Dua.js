...
const Dua = ({ catId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`http://localhost:5000/duas/${catId}`);
        const data = await res.json();
        setDuaByCategory(data);
      } catch (error) {
  ...
  return (
    <>
      {duaByCategory?.map((dua) => (
        <div key={dua?.id} className="bg-white px-6 py-5 mb-5 rounded-xl">
          {/* dua logo and title  */}
          ...
          <div className="text-sm text-justify">
            <p className="mb-7 leading-8">{dua?.top_en}</p>
